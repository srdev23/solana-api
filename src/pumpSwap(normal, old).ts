import {
  ...
  import { bufferFromUInt64, getCoinData } from "./utils";
  import {
    GLOBAL,
    FEE_RECIPIENT,
    SYSTEM_PROGRAM_ID,
  ...
  const swapPumpToken = async (req: Request, res: Response, isBuy: boolean) => {
    ...
    try {
      ...
      if (!coinData) {
        ...
        return res
        .status(500)
        .json({ status: "failed", message: "Failed to retrieve pump token data" });
      }
    ...
    //   txn.sign([userKeypair]);
      ...
      const sig = await sendAndConfirmTransaction(connection, transaction, [userKeypair], { skipPreflight: true, preflightCommitment: 'confirmed' });
