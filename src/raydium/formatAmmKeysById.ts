...
import { connection } from "../../config";  // Use the connection from config.js

// Function to fetch pool info using a mint address
export async function fetchPoolInfoByMint(mint: string): Promise<ApiPoolInfoV4> {
  try {
...
async function decodePoolAndMarketInfo(poolAccount, mint): Promise<ApiPoolInfoV4> {
